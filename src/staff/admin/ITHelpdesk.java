...
public class ITHelpdesk extends Staff{
	...
	public ITHelpdesk(String staffName, String staffSurname, String gender, int workFunction, double salaryLevel,
...
//start to load the "queue".
			printer.typeWriter("Grand, let's see how many animals you need to update in the system today", 40);
...
//Producing the effect to "adding" and "removing" animals
			for (Animals animal : ListWork) {
				System.out.println(animal);
				query.YesOrNo(2,
						"Please type   *yes or no*    \n to mark the registration up-to-date status. \n\n yes - Done        no - Unresolved");
				if (query.getUserYesOrNoValidOptio() == 1) {
					workDone.add(animal);
...
//			give a chance the user finish the list again.
				...
				if (query.getUserYesOrNoValidOptio() == 1) {
					...
					for (Animals animal : workUnresolved) {
						System.out.println(animal);
						query.YesOrNo(2,
								"Please type   *yes or no*    \n to mark registration up-to-date status. \n\n yes - Done        no - Unresolved");
						if (query.getUserYesOrNoValidOptio() == 1) {
							workDone.add(animal);
...
//	we could make a statement to clear the records on listWork but we decide leave as it is.
	...
	public static void setWorkUnresolved(List<Animals> workUnresolved) {
		ITHelpdesk.workUnresolved = workUnresolved;
	}	
